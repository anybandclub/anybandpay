...
*Setup instructions for a Gitian build of Abp Core using a Debian VM or physical system.*
...
- [Building Abp Core](#building-abp-core)
...
Clone the git repositories for abp and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/abpproject/abp --recursive
git clone https://github.com/abpproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/abp/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the abp repository under 'Fetch and create inputs' to install sources which require
...
To build Abp Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the abp repository.
...
For example:
```bash
URL=https://github.com/laanwj/abp.git
...
# if wanting to use a different version of eth-cpp-abp:
./bin/gbuild --commit abp=${COMMIT},cpp-eth-abp=${ETHCOMMIT} --url abp=${URL},cpp-eth-abp=${ETHURL} ../abp/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the abp git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../abp/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/abpproject/abp-detached-sigs.git

BTCPATH=/some/root/path/abp
SIGPATH=/some/root/path/abp-detached-sigs

./bin/gbuild --url abp=${BTCPATH},signature=${SIGPATH} ../abp/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/abp-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/abp-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/abp-osx-build.assert
```
...
[abpproject/gitian.sigs](https://github.com/abpproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@abp.org) and he will commit them.