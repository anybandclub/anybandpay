...
UniValue blockheaderToJSON(const CBlockIndex* blockindex)
{
    ...
    result.push_back(Pair("hashStateRoot", blockindex->hashStateRoot.GetHex())); // abp
    result.push_back(Pair("hashUTXORoot", blockindex->hashUTXORoot.GetHex())); // abp
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.push_back(Pair("hashStateRoot", block.hashStateRoot.GetHex())); // abp
    result.push_back(Pair("hashUTXORoot", block.hashUTXORoot.GetHex())); // abp
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() == 3){
        CTxDestination abpSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(abpSenderAddress)) {
            const CKeyID *keyid = boost::get<CKeyID>(&abpSenderAddress);
