...
void initState(){
    ...
    const std::string dirAbp = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<AbpState>(new AbpState(dev::u256(0), AbpState::openDB(dirAbp, hashDB, dev::WithExisting::Trust), dirAbp + "/abpDB", dev::eth::BaseState::Empty));
...
AbpTransaction createAbpTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout){
    AbpTransaction txEth;
    if(recipient == dev::Address()){
        txEth = AbpTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = AbpTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<AbpTransaction> txs){
