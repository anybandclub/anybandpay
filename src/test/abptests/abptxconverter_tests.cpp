...
void checkResult(bool isCreation, std::vector<AbpTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    AbpTxConverter converter(transaction, NULL);
    ExtractAbpTX abpTx;
    BOOST_CHECK(converter.extractionAbpTransactions(abpTx));
    std::vector<AbpTransaction> result = abpTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(abptxconverter_tests, TestingSetup)
