...
std::unique_ptr<AbpState> globalState;
...
const std::string strMessageMagic = "Abp Signed Message:\n";
                              ...
                              bool bypass_limits, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool rawTx)
{
...

    {
        ...
        if(tx.HasCreateOrCall()){
            ...
            AbpDGP abpDGP(globalState.get(), fGettingValuesDGP);
            uint64_t minGasPrice = abpDGP.getMinGasPrice(chainActive.Tip()->nHeight + 1);
            uint64_t blockGasLimit = abpDGP.getBlockGasLimit(chainActive.Tip()->nHeight + 1);
            size_t count = 0;
            for(const CTxOut& o : tx.vout)
                count += o.scriptPubKey.HasOpCreate() || o.scriptPubKey.HasOpCall() ? 1 : 0;
            AbpTxConverter converter(tx, NULL);
            ExtractAbpTX resultConverter;
            if(!converter.extractionAbpTransactions(resultConverter)){
            ...
            std::vector<AbpTransaction> abpTransactions = resultConverter.first;
            std::vector<EthTransactionParams> abpETP = resultConverter.second;

            dev::u256 sumGas = dev::u256(0);
            dev::u256 gasAllTxs = dev::u256(0);
            for(AbpTransaction abpTransaction : abpTransactions){
                sumGas += abpTransaction.gas() * abpTransaction.gasPrice();
                ...
                if(txMinGasPrice != 0) {
                    txMinGasPrice = std::min(txMinGasPrice, abpTransaction.gasPrice());
                } else {
                    txMinGasPrice = abpTransaction.gasPrice();
                }
                VersionVM v = abpTransaction.getVersion();
                if(v.format!=0)
                ...
                if(abpTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
                ...
                if(abpTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
                ...
                if(abpTransaction.gas() > UINT32_MAX)
                ...
                if(v.rootVM!=0 && (uint64_t)abpTransaction.gasPrice() < minGasPrice)
            ...
            if(!CheckMinGasPrice(abpETP, minGasPrice))
                return state.DoS(100, false, REJECT_INVALID, "bad-txns-small-gasprice");

            if(count > abpTransactions.size())
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s ABP additional fees, %d delta bytes\n",
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    if(nHeight <= consensusParams.nLastPOWBlock)
        return 1132800 * COIN; //abp 20000 * COIN;
    ...
    CAmount nSubsidy = 3 * COIN; //abp 3 * COIN;
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // abp
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // abp
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    AbpTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<AbpTransaction>(1, callTransaction), blockGasLimit);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path abpDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(abpDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(abpDir.string(), std::ios::in | std::ios::out);
...
bool AbpTxConverter::extractionAbpTransactions(ExtractAbpTX& abptx){
    std::vector<AbpTransaction> resultTX;
    ...
    abptx = std::make_pair(resultTX, resultETP);
    return true;
}

bool AbpTxConverter::receiveStack(const CScript& scriptPubKey){
...
bool AbpTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
AbpTransaction AbpTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    AbpTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = AbpTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = AbpTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // abp
    AbpDGP abpDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setAbpSchedule(abpDGP.getGasSchedule(pindex->nHeight + 1));
    uint32_t sizeBlockDGP = abpDGP.getBlockSize(pindex->nHeight + 1);
    uint64_t minGasPrice = abpDGP.getMinGasPrice(pindex->nHeight + 1);
    uint64_t blockGasLimit = abpDGP.getBlockGasLimit(pindex->nHeight + 1);
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, SER_NETWORK, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // abp
...
///////////////////////////////////////////////////////////////////////////////////////// abp
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            AbpTxConverter convert(tx, &view, &block.vtx);

            ExtractAbpTX resultConvertAbpTX;
            if(!convert.extractionAbpTransactions(resultConvertAbpTX)){
            ...
            if(!CheckMinGasPrice(resultConvertAbpTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertAbpTX.first, blockGasLimit);
            ...
            for(AbpTransaction& qtx : resultConvertAbpTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertAbpTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{block.GetHash(), uint32_t(pindex->nHeight), tx.GetHash(), uint32_t(i), resultConvertAbpTX.first[k].from(), resultConvertAbpTX.first[k].to(),
...
//////////////////////////////////////////////////////////////////
    ...
    if(pindex->nHeight > 5000 && !Params().GetConsensus().fPoSNoRetargeting) {
        ...
        if(pindex->nMoneySupply > (uint64_t)(5664000000 + ((pindex->nHeight - 5000) * 3)) * COIN){ //abp (uint64_t)(100000000 + ((pindex->nHeight - 5000) * 4)) * COIN){
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // abp
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // abp
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // abp
            globalState->setRootUTXO(oldHashUTXORoot); // abp
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev)
    {
        ...
        ///////////////////////////////////////////////////////////////////// // abp
        uint32_t sizeBlockDGP = abpDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for Abp PoS we need to sync this after every block to ensure txdb is populated for
