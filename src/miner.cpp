...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setAbpSchedule(abpDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = abpDGP.getBlockSize(nHeight);
    minGasPrice = abpDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = abpDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    AbpTxConverter convert(iter->GetTx(), NULL, &pblock->vtx);

    ExtractAbpTX resultConverter;
    if(!convert.extractionAbpTransactions(resultConverter)){
    ...
    for(AbpTransaction abpTransaction : abpTransactions){
        txGas += abpTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + abpTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(abpTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, abpTransactions, hardBlockGasLimit);
...
void StakeAbps(bool fStake, CWallet *pwallet)
{
