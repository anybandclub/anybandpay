...
#include <abp/storageresults.h>


extern std::unique_ptr<AbpState> globalState;
...
using ExtractAbpTX = std::pair<std::vector<AbpTransaction>, std::vector<EthTransactionParams>>;
///////////////////////////////////////////
...
static const size_t MAX_CONTRACT_VOUTS = 1000; // abp
...
void EnforceContractVoutLimit(ByteCodeExecResult& bcer, ByteCodeExecResult& bcerOut, const dev::h256& oldHashAbpRoot,
    const dev::h256& oldHashStateRoot, const std::vector<AbpTransaction>& transactions);
...
class AbpTxConverter{

public:

    AbpTxConverter(CTransaction tx, CCoinsViewCache* v = NULL, const std::vector<CTransactionRef>* blockTxs = NULL) : txBit(tx), view(v), blockTransactions(blockTxs){}

    bool extractionAbpTransactions(ExtractAbpTX& abpTx);
...
private:
    ...
    AbpTransaction createEthTX(const EthTransactionParams& etp, const uint32_t nOut);
...
public:
    ...
    ByteCodeExec(const CBlock& _block, std::vector<AbpTransaction> _txs, const uint64_t _blockGasLimit) : txs(_txs), block(_block), blockGasLimit(_blockGasLimit) {}
