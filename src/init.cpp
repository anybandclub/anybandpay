...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/abpproject/abp>";
    const std::string URL_WEBSITE = "<https://abp.org>";
    ...
    while (!fLoaded && !fRequestShutdown) {
        ...
        do {
            try {
                ...
                dev::eth::Ethash::init();
                fs::path abpStateDir = GetDataDir() / "stateAbp";
                bool fStatus = fs::exists(abpStateDir);
                const std::string dirAbp(abpStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsAbpstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<AbpState>(new AbpState(dev::u256(0), AbpState::openDB(dirAbp, hashDB, dev::WithExisting::Trust), dirAbp, existsAbpstate));
                dev::eth::ChainParams cp((dev::eth::genesisInfo(dev::eth::Network::abpMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(abpStateDir.string()));
                if (fReset) {
