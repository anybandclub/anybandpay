...
class AbpState : public dev::eth::State {
    
public:

    AbpState();

    AbpState(dev::u256 const& _accountStartNonce, dev::OverlayDB const& _db, const std::string& _path, dev::eth::BaseState _bs = dev::eth::BaseState::PreExisting);

    ResultExecute execute(dev::eth::EnvInfo const& _envInfo, dev::eth::SealEngineFace const& _sealEngine, AbpTransaction const& _t, dev::eth::Permanence _p = dev::eth::Permanence::Committed, dev::eth::OnOpFunc const& _onOp = OnOpFunc());
...
private:
    ...
    dev::Address createAbpAddress(dev::h256 hashTx, uint32_t voutNumber);
...
struct TemporaryState{
    std::unique_ptr<AbpState>& globalStateRef;
    dev::h256 oldHashStateRoot;
    dev::h256 oldHashUTXORoot;

    TemporaryState(std::unique_ptr<AbpState>& _globalStateRef) : 
...
public:
    ...
    CondensingTX(AbpState* _state, const std::vector<TransferInfo>& _transfers, const AbpTransaction& _transaction, std::set<dev::Address> _deleteAddresses = std::set<dev::Address>()) : transfers(_transfers), deleteAddresses(_deleteAddresses), transaction(_transaction), state(_state){}
...
private:
    ...
    const AbpTransaction& transaction;
