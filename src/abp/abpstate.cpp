...
#include <abp/abpstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

AbpState::AbpState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = AbpState::openDB(_path + "/abpDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

AbpState::AbpState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute AbpState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, AbpTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createAbpAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            abp::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
...
std::unordered_map<dev::Address, Vin> AbpState::vins() const // temp
{
...
void AbpState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* AbpState::vin(dev::Address const& _a) const
{
    return const_cast<AbpState*>(this)->vin(_a);
}

Vin* AbpState::vin(dev::Address const& _addr)
{
...
// void AbpState::commit(CommitBehaviour _commitBehaviour)
// {
...
void AbpState::kill(dev::Address _addr)
{
...
void AbpState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
dev::Address AbpState::createAbpAddress(dev::h256 hashTx, uint32_t voutNumber){
...
void AbpState::deleteAccounts(std::set<dev::Address>& addrs){
...
void AbpState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void AbpState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
