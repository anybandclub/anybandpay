#include <abp/abpDGP.h>

void AbpDGP::initDataEIP158(){
...
bool AbpDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData){
...
dev::eth::EVMSchedule AbpDGP::getGasSchedule(unsigned int blockHeight){
    clear();
...
uint64_t AbpDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t AbpDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t AbpDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t AbpDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool AbpDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void AbpDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void AbpDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void AbpDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void AbpDGP::createParamsInstance(){
...
dev::Address AbpDGP::getAddressForBlock(unsigned int blockHeight){
...
void AbpDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AbpDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void AbpDGP::parseStorageOneUint64(uint64_t& value){
...
void AbpDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule AbpDGP::createEVMSchedule(){
    dev::eth::EVMSchedule schedule = dev::eth::EIP158Schedule;
