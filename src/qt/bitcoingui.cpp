...
#include <qt/abpversionchecker.h>
    ...
    platformStyle(_platformStyle)
{
    ...
    abpVersionChecker = new AbpVersionChecker(this);

    if(fCheckForUpdates && abpVersionChecker->newVersionAvailable())
    {
        QString link = QString("<a href=%1>%2</a>").arg(ABP_RELEASES, ABP_RELEASES);
        QString message(tr("New version of Abp wallet is available on the Abp source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::createActions()
{
    ...
    openAction->setStatusTip(tr("Open a abp: URI or payment request"));//ABP_LINE

    showHelpMessageAction = new QAction(platformStyle->MenuColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Abp command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Abp network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
{
    QString strTitle = tr("Abp"); // default title
