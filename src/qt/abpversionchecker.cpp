...
#define paternVersion "abp-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

AbpVersionChecker::AbpVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

AbpVersionChecker::~AbpVersionChecker()
{

}

bool AbpVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> AbpVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(ABP_RELEASES)));
    QEventLoop event;
...
Version AbpVersionChecker::getMaxReleaseVersion()
{
