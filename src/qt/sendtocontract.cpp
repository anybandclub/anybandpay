    ...
    m_results(1)
{
    ...
    ui->labelAmount->setToolTip(tr("The amount in ABP to send. Default = 0."));
...
void SendToContract::on_numBlocksChanged()
{
    if(m_clientModel)
    {
        ...
        ui->labelGasPrice->setToolTip(tr("Gas price: ABP price per gas unit. Default = %1, Min = %2.").arg(QString::fromStdString(FormatMoney(DEFAULT_GAS_PRICE))).arg(QString::fromStdString(FormatMoney(minGasPrice))));
