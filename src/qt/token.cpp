...
bool ToHash160(const std::string& strAbpAddress, std::string& strHash160)
{
    CTxDestination abpAddress = DecodeDestination(strAbpAddress);
    if(!IsValidDestination(abpAddress))
        return false;
    const CKeyID * keyid = boost::get<CKeyID>(&abpAddress);
    if(keyid){
...
bool ToAbpAddress(const std::string& strHash160, std::string& strAbpAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    CKeyID keyid(key);
    CTxDestination abpAddress = keyid;
    if(IsValidDestination(abpAddress)){
        strAbpAddress = EncodeDestination(abpAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToAbpAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToAbpAddress(tokenEvent.receiver, tokenEvent.receiver);
